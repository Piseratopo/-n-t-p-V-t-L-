...
\usepackage{amsmath, amssymb, blindtext, calc, hyperref, icomma, multicol,  steinmetz, tikz, unicode-math, wrapfig, xcolor, xparse, xeCJK}
...
\NewDocumentCommand{\exercise}{o}{
   ...
   \IfNoValueTF{#1}
     {\label{ex\arabic{exercise}}}  % Default label
     {\label{#1}}                   % Custom label
}
...
\NewDocumentCommand{\solution}{o}{
   ...
   \IfNoValueTF{#1}
     {\label{sol\arabic{solution}}}  % Default label
     {\label{#1}}                    % Custom label
}
...
\begin{wrapfigure}{r}{0.4\textwidth}
   ...
   \begin{tikzpicture}
      ...
      \draw (1,0) -- (1,-0.08) node[below] {$a$};
      \draw (0,1.5) -- (-0.08,1.5) node[left] {$b$};

      \filldraw (1, 1.5) circle (1.5pt);
      \node[above right] at (1, 1.5) {$P(a; b)$};
