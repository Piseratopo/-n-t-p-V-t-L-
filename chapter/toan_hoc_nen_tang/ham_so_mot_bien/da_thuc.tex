...
\defText{Hàm đa thức} thông thường được biểu diễn dưới dạng tổng của các đơn thức $$\defMath{f(x)=P_n(x)=\sum_{i = 0}^n \left(a_i x^i\right) = a_nx^n + a_{n-1}x^{n-1} + \cdots + a_1x + a_0}$$ với $n$ là một số nguyên không âm, $a_i$ là các số thực, gọi là các \defText{hệ số}, với mọi $i$ nguyên nằm trong đoạn $[0, n]$ và $a_n \neq 0$. Khi này, $n$ được gọi là \defText{bậc} của đa thức\label{def:ham_so_mot_bien:da_thuc:da_thuc}. Mọi giá trị $x \in \mathbb{R}$ đều thuộc tập xác định của hàm đa thức $f(x)$. Ví dụ:
\begin{itemize}
   ...
   \begin{minipageindent}{0.4\textwidth}
      \begin{figure}[H]
         ...
         \begin{tikzpicture}
            ...
            \draw[graph thickness, color=colorEmphasis, domain=-1.75:-1] plot (\x, {4*\x + 7});
            \filldraw[color=colorEmphasis] (-1.5, 1) circle (\pointSize) node[right] {$\left(-\frac{3}{2}; 1\right)$};
            ...
            \draw[color=colorEmphasis, graph thickness, fill=white] (-1.75, 0) circle (\pointSize) node[below right] {$\left(-\frac{7}{4}; 0\right)$};
         \end{tikzpicture}
         \caption{Đồ thị biểu diễn $f(x) = 4x + 7$ và khoảng nhỏ hơn $0$.}
   ...
   \begin{minipageindent}{0.4\textwidth}
      \begin{figure}[H]
         ...
         \begin{tikzpicture}
            ...
            \draw[graph thickness, color=colorEmphasis, domain=-2:2] plot (\x, {-(\x) - 2});
            \draw[color=colorEmphasis, graph thickness, fill=white] (-2, 0) circle (\pointSize) node[above right] {$\left(-2; 0\right)$};
            \filldraw[color=colorEmphasis] (0, -2) circle (\pointSize) node[above right] {$\left(0; -16\right)$};
         \end{tikzpicture}
         \caption{Đồ thị biểu diễn $f(x) = -8x - 16$ và khoảng lớn hơn $0$.}
   ...
   \begin{minipageindent}{0.4\textwidth}
      \begin{figure}[H]
         ...
         \begin{tikzpicture}
            ...
            \draw[graph thickness, color=colorEmphasisCyan, domain=2.333:4] plot (\x, {((\x) - 2) / 2});
            \draw[graph thickness, color=colorEmphasis, domain=0:2.333] plot (\x, {((\x) - 2) / 2});
            \draw[graph thickness, color=colorEmphasisCyan, domain=2.333:4] plot (\x, {(-2 * (\x) + 5) / 2});
            \draw[graph thickness, color=colorEmphasis, domain=0:2.333] plot (\x, {(-2 * (\x) + 5) / 2});
            \filldraw[color=colorEmphasisCyan] (2.333, {0.333 / 2}) circle (\pointSize);
            \node[right, color=colorEmphasisCyan] at (2.4, 0.1) {$\left(\frac{7}{3}; \frac{1}{3}\right)$};
            \filldraw[color=colorEmphasis] (0, {5 / 2}) circle (\pointSize) node[right] {$\left(0; \frac{5}{2}\right)$};
...
4. Để ý rằng $x^2 + 6x + 10 = \left(x^2 + 6x + 9\right) + 1 = (x + 3)^2 + 1 \implies x^2 + 6x + 10 \geq 1$. Do đó, $x^2 + 6x + 10 \leq 0$ không có nghiệm.
