...
% }
% \hline
% \textbf{Khoảng} & \textbf{Biểu diễn} \\
% \hline
% Dòng một có thể dài nên sẽ tự xuống dòng
% & Nội dung cột hai cũng được căn giữa cả ngang và dọc \\
% \hline
% Nội dung hàng 2 & Cột 2 hàng 2 \\
% \hline
% \end{tabular}
   ...
   %    \textbf{Khoảng} & \textbf{Biểu diễn} 
      ...
      %    \begin{tikzpicture}
      %       \draw[->] (-3, 0) -- (3, 0);
      %       \filldraw[color=colorEmphasisCyan] (-1.2, 0) circle (\pointSize);
      %       \filldraw[color=colorEmphasisCyan] (1.8, 0) circle (\pointSize);
      %       \draw[graph thickness,color=colorEmphasisCyan] (-1.2, 0) -- (1.8, 0);
      %    \end{tikzpicture}
      %    \\
      %    \hline
      %    \defMath{\left[a; b\right) = \left[a; b\right[ = \left\{x \in \mathbb{R} : a \leq x < b\right\}}
...
\begin{itemize}
   \item ;
   \item $\defMath{\left(a; b\right] = \left]a; b\right] = \left\{x \in \mathbb{R} : a < x \leq b\right\}}$;
   \item $\defMath{\left(a; b\right) = \left]a; b\right[ = \left\{x \in \mathbb{R} : a < x < b\right\}}$;
   \item $\defMath{\left(-\infty; a\right] = \left]-\infty; a\right] = \left\{x \in \mathbb{R} : x \leq a\right\}}$;
   \item $\defMath{\left(-\infty; a\right) = \left]-\infty; a\right[ = \left\{x \in \mathbb{R} : x < a\right\}}$;
