...
Chỉ có $5$ phép nối mệnh đề như sau:
\begin{itemize}
   \item Phép \defText{đối} --- $\defMath{\neg}$,
   \item Phép \defText{hội} hoặc phép \defText{và} --- $\defMath{\land}$,
   \item Phép \defText{tuyển} hoặc phép \defText{hoặc} --- $\defMath{\lor}$,
   \item Phép \defText{kéo theo} --- $\defMath{\implies}$ hoặc $\defMath{\Rightarrow}$,
   \item Phép \defText{tương đương} --- $\defMath{\iff}$ hoặc $\defMath{\Leftrightarrow}$.
\end{itemize}
...
\begin{enumerate}
   ...
   \item Phép và --- $\land$;
   \item Phép hoặc --- $\lor$;
   \item Phép kéo theo và phép tương đương --- $\implies$ và $\iff$ (mức ưu tiên thấp nhất).
\end{enumerate}
...
\begin{table}[H]
   \centering
   \caption{Bảng giá trị chân lí cho bài \ref{ex:ex:toan_hoc_nen_tang:lo_gich:menh_de_phuc_hop:vd_bang_gia_tri_chan_li} phần \arabic{subexercise}}
...
\begin{minipage}[c]{0.68\linewidth}
   \centering
   \captionof{table}{Bảng giá trị chân lí rút gọn cho bài 
   \ref{ex:ex:toan_hoc_nen_tang:lo_gich:menh_de_phuc_hop:vd_bang_gia_tri_chan_li} 
   phần \arabic{subexercise}}
...
\exercise Sử dụng bảng giá trị chân lí, chứng minh các mệnh đề sau luôn đúng với mọi giá trị chân lí của $P$, $Q$ và $R$.
\begin{itemize}
   \item $P \lor \neg P$ (tính loại trừ trung gian\footnote{Một mệnh đề hoặc đúng, hoặc phủ định của nó đúng; không tồn tại khả năng trung gian.});
   \item $\neg\left(P \land \neg P\right)$ (tính không mâu thuẫn\footnote{Không thể có một mệnh đề vừa đúng vừa sai cùng lúc và theo cùng một nghĩa.});
   \item $\neg\left(\neg P\right) \iff P$ (tính phủ định kép);
   \item $P \land P \iff P$ (tính khả nghịch với phép hội);
   \item $P \lor P \iff P$ (tính khả nghịch với phép tuyển);
   \item $P \land Q \iff Q \land P$ (tính giao hoán với phép hội);
   \item $P \lor Q \iff Q \lor P$ (tính giao hoán với phép tuyển);
   \item $\neg\left(P\lor Q\right) \iff \neg P \land \neg Q$ (định luật Đờ Moóc-gơn\footnote{Augustus De Morgan (1806 -- 1871)}, phần 1);
   \item $\neg\left(P\land Q\right) \iff \neg P \lor \neg Q$ (định luật Đờ Moóc-gơn, phần 2);
   \item $P \implies Q \iff \neg P \lor Q$;
   \item $\neg\left(P \implies Q\right) \iff \left(\neg Q \implies \neg P\right)$ (phép đối trên phép kéo theo);
   \item $\left(P \iff Q\right) \iff \left(P \implies Q\right) \land \left(Q \implies P\right)$;
