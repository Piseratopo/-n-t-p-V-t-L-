...
\begin{itemize}
   ...
   \item Phép \defText{tuyển} hoặc phép \defText{hoặc} --- $\defMath{\lor}$, $\defMath{\parallel}$, $\defMath{+}$;
   \item Phép \defText{kéo theo} --- $\defMath{\implies}$ hoặc $\defMath{\Rightarrow}$, $\defMath{\rightarrow}$, $\defMath{\supset}$;
   \item Phép \defText{đẳng giá} hoặc phép \defText{tương đương lô-gích} --- $\defMath{\iff}$ hoặc $\defMath{\Leftrightarrow}$, $\defMath{\leftrightarrow}$, $\defMath{\equiv}$.
\end{itemize}
