...
\subsection{Quy tắc cụ thể hóa và quy tắc tồn tại hóa}

\ 

Bất cứ khi nào chúng ta nói rằng ``tồn tại một $x$ thỏa mãn một tính chất $A(x)$ nào đó'', điều đó có nghĩa rằng $x$ có thể là một $c$ nào đó có tính chất $A(c)$. Chúng ta không biết, và cũng không cần phải biết, $c$ có hình thù ra làm sao, tên tuổi như thế nào. Điều hiển nhiên này được gọi là \defText{quy tắc cụ thể hóa}. Dù nghe có vẻ hiển nhiên, việc chính thống hóa nó là một bước ngoặt quan trọng trong lập luận. Nó cung cấp cho chúng ta một cái tên tạm để đặt nó vào bàn cân suy luận và từ đó liên kết thêm các định luật khác và khám phá ra những thuộc tính mới.
...
\begin{enumerate}
    \item $\forall x, A(x) \lor \forall x, B(x) \implies \forall x, \left(A(x) \lor B(x)\right)$ (tính chất phân phối một chiều của lượng từ, phần 1);
    \item $\exists x, \left(A(x) \land B(x)\right) \implies \exists x, A(x) \land \exists x, B(x)$ (tính chất phân phối một chiều của lượng từ, phần 2);
    ...
    \item $\exists x, \forall y, A(x;y) \implies \forall y, \exists x, A(x; y)$ (quy tắc trao đổi lượng từ).
\end{enumerate}

\solution

\setcounter{subexercise}{1}
\arabic{subexercise}. Khi có $\forall x, A(x) \lor \forall x, B(x)$, chúng ta xét hai trường hợp:
\begin{itemize}
    \item \textcolor{colorEmphasisCyan}{Trường hợp 1 --- $\forall x, A(x)$}: Do có $P \implies P \lor Q$ đúng với mọi mệnh đề $P$ và $Q$ theo tính chất cộng, cho nên $\forall x, A(x) \implies \forall x, \left(A(x) \lor B(x)\right)$.
    \item \textcolor{colorEmphasis}{Trường hợp 2 --- $\forall x, B(x)$}: Với một lập luận hoàn toàn tương tự, chúng ta cũng có $\forall x, B(x) \implies \forall x, \left(A(x) \lor B(x)\right)$.
\end{itemize}

Kết hợp hai trường hợp, chúng ta có
\begin{equation*}
    \forall x, A(x) \lor \forall x, B(x)  \implies \forall x, \left(A(x) \lor B(x)\right).
\end{equation*}
Vậy, chúng ta có điều phải chứng minh.

\stepcounter{subexercise}
\arabic{subexercise}. Theo quy tắc cụ thể hóa, nếu có $\exists x, \left(A(x) \land B(x)\right)$, thì tồn tại $c$ để
\begin{equation*}
    A(c) \land B(c).
\end{equation*}
Hiển nhiên, theo tính chất rút gọn, chúng ta có
\begin{equation*}
    \begin{cases}
        A(c) \land B(c) \implies A(c) \\
        A(c) \land B(c) \implies B(c)
    \end{cases}
\end{equation*}
