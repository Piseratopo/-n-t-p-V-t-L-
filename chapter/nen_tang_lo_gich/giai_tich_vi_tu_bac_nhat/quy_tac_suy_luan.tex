...
\begin{multicols}{2}
    \begin{enumerate}
        ...
        \item $\forall x, \forall y, \big(V_<(x, y) \implies V_\le(x, y))$;
...
\stepcounter{subexercise}
\arabic{subexercise}.
Chúng ta có thể khẳng định sự hiển nhiên của $(P \lor Q) \land \neg P \implies Q$ cửa bảng giá trị chân lí \ref{tab:nen_tang_lo_gich:giai_tich_vi_tu_bac_nhat:quy_tac_suy_luan:3.1}.
\begin{table}[H]
	\centering
	\caption{Bảng giá trị chân lí của $( P \lor Q ) \land \neg Q \implies P$}
    \label{tab:nen_tang_lo_gich:giai_tich_vi_tu_bac_nhat:quy_tac_suy_luan:3.1}

	\begin{tabular}{|c|c|cccccccc!{\vrule width 1.5pt}c|}
		\hline
		$P$ & $Q$ & $(P$ & $\lor$ & $Q)$ & $\land$ & $\neg$ & $Q$ & $\implies$ & $P$ & $( P \lor Q ) \land \neg Q \implies P$\\
		\headerDivider
		Đ & Đ & Đ & Đ & Đ & S & S & Đ & \emphcolor{Đ} & Đ & \multirow{4}{*}{\huge\vspace{-12pt}Đ}\\
	...
	\end{tabular}
\end{table}
Sử dụng quy tắc thay thế, đổi $P$ và $Q$ lần lượt bởi $V_\le(x, y)$ và $V_\le(y, x)$, chúng ta có
\begin{equation*}
    G \implies \forall x, \forall y, \big((V_\le(x, y) \lor V_\le(y, x) )\land \neg V_\le(y, x) \implies V_\le(x, y)\big). 
\end{equation*}
...
\end{equation*}
Qua đó, kết hợp hai mệnh đề, sử dụng thay thế cơ bản để có
\begin{align*}
    G &\implies \forall x, \forall y, \big((V_\le(x, y) \lor V_\le(y, x))\land V_<(x, y) \implies V_\le(x, y)\big); \\ 
    G &\implies \forall x, \forall y, \big(V_<(x, y) \implies V_\le(x, y)\big) \equationexplanation{tính chất rút gọn}.
\end{align*}
Điều phải chứng minh.
